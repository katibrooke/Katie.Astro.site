...
with col1:
    # Исправленный диапазон дат (от 1900 года)
    d = st.date_input(
        "Дата рождения", 
        format="DD/MM/YYYY",
    ...
    # Исправленный шаг времени (теперь можно выбрать любую минуту)
    t = st.time_input("Время рождения", step=60)
with col2:
    # Акцент на английском языке
    city = st.text_input("Город (ТОЛЬКО на английском, например: Tel Aviv)")

if st.button("Рассчитать"):
    if not city:
        st.error("Пожалуйста, введите город на английском.")
    else:
        try:
            ...
            if loc:
                ...
                st.success(f"Расчет выполнен для: {city}")
                ...
                st.error("Город не найден. Убедитесь, что название написано на английском.")
        except Exception as e:
            st.error("Произошла техническая ошибка. Попробуйте ввести город Tel Aviv для проверки.")
