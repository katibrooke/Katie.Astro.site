...
st.markdown("""
    ...
    .result-card {
        ...
        margin-bottom: 10px; color: #4a4a4a; font-family: 'Arial';
    }
    .asc-card {
...
# --- УТИЛИТА ПОИСКА ДОМА (Стабильная версия) ---
def get_house(lon, cusps):
    for i in range(1, 12):
        if cusps[i] < cusps[i+1]:
            if cusps[i] <= lon < cusps[i+1]: return i
        else: # Переход через 0 градусов
            if lon >= cusps[i] or lon < cusps[i+1]: return i
    return 12
...
# ИМЯ РЕБЕНКА
user_name = st.text_input("Имя (например: Валера или Анна)", value="Мой ребенок")

col1, col2 = st.columns(2)
with col1:
    d = st.date_input("Дата рождения", format="DD/MM/YYYY", min_value=datetime(1900, 1, 1))
    t_in = st.text_input("Время (например: 12:17)", value="12:00")
with col2:
...
if st.button("Рассчитать карту"):
    ...
    try:
        with st.spinner('Считываю энергию звезд...'):
            geolocator = Nominatim(user_agent="katy_astro_final_fixed")
            ...
            if loc:
                ...
                st.download_button("📸 Скачать именную карту в галерею", img_bin, f"{user_name}_astro.png", "image/png")
            else: st.error("Город не найден.")
    except Exception as e: 
        st.error("Техническая ошибка в расчетах. Попробуйте еще раз.")
